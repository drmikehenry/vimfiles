...
function! s:on_exit(id_or_channel) dict abort
  ...
  execute (self.flags.quickfix ? 'caddexpr' : 'laddexpr') 'self.stdoutbuf'
...
function! s:run(flags)
  ...
  execute 'silent' (a:flags.quickfix ? 'cgetexpr' : 'lgetexpr') " ''"
  redraw
  echo printf('%s', 'Grepper: ' . s:cmdline)
  ...
  elseif !get(w:, 'testing') && (v:version > 704 || v:version == 704 && has('patch1967'))
    ...
    execute 'silent' (a:flags.quickfix ? 'caddexpr' : 'laddexpr') 'system(s:cmdline)'
