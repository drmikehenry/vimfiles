...
" Extra hacks:	2007-12-19: <marius@gedmin.as> added support for aligning
"               attributes like
"                       <img src="..."
"                            alt="...">
"                       more text
"               2010-09-27: <marius@gedmin.as> added ^F to indentkeys
" Globals:	g:html_indent_tags	   -- indenting tags
"		g:html_indent_strict       -- inhibit 'O O' elements
"		g:html_indent_strict_table -- inhibit 'O -' elements
"		g:html_indent_attrs        -- indenting tag attributes
...
if !exists('g:html_indent_attrs')
    let g:html_indent_attrs=1
endif
...
fun! <SID>HtmlIndentOpen(lnum, pattern)
    " try to ignore empty xhtml tags like <div />
    let s = substitute(getline(a:lnum),
          \ '<\('.a:pattern.'\)\>\([^/>"'']\|"[^"]*"\|''[^'']*''\)*/>', '', 'g')
    let s = substitute('x'.s,
    \ '.\{-}\(\(<\)\('.a:pattern.'\)\>\)', "\1", 'g')
...
fun! HtmlIndentGet(lnum)
    ...
    if g:html_indent_attrs
	" tags split over multiple lines, e.g.:
	"    <img src="..."
	"	  alt="...">
	if getline(lnum) =~ '<[^<>]*$'
	    let s = substitute(getline(lnum), '<\w\+\s\+\zs[^<>]*$', '', '')
	    return strlen(s)
	endif
	if getline(lnum) =~ '^[^<>]*>'
	    let lnum = search('<[^<>]*$', 'bW')
